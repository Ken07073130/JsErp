<%@ Page Language="C#" AutoEventWireup="true" CodeFile="cdgl.aspx.cs" Inherits="stcd_cdgl" %>
...
<script type="text/javascript">
    ...
    //提交前检查输入情况
    function IntoClick(){
        var cm=document.getElementById('<%= T_CM.ClientID %>').value
            if(cm==""){
                alert("菜名不能为空！");
                return false;
            }
        var dj=document.getElementById('<%= T_DJ.ClientID %>').value
            if(dj==""){
                alert("单价不能为空！");
                return false;
            }
        var sl=document.getElementById('<%= T_SL.ClientID %>').value
            if(sl==""){
                alert("数量不能为空！");
                return false;
            }
    ...
    function cmcz(cm_dj){
        if(cm_dj!=""){
            var cmdj=cm_dj.split("|");
            for(var i=0;i<cmdj.length;i++)
            {
                if (i==0){
                    document.getElementById('<%= T_CM.ClientID %>').value=cmdj[i];
                }
                if (i==1){
                    document.getElementById('<%= T_DJ.ClientID %>').value=cmdj[i];
                }
    ...
    <form id="form1" runat="server">
        ...
        <ajaxToolkit:AutoCompleteExtender ID="AutoCompleteExtender1" runat="server"  TargetControlID="T_CM" ServicePath="../GetContent.asmx" ServiceMethod="GetcmList" MinimumPrefixLength="1"  CompletionInterval="200" />
        <ajaxToolkit:CalendarExtender ID="CalendarExtender1" runat="server"  TargetControlID="T_RQ" Format="yyyy-MM-dd" PopupPosition="TopRight"  />
	    ...
	    <table width="100%" style="height:580px" border="0" cellspacing="0" cellpadding="0">
          <tr >
            ...
            <td valign="top" align="center"  style="height:100%; width: 1125px;" >
                <strong><span style="font-size: 20pt">每日菜单编辑<span style="font-size: 9pt"><a href="sldb.aspx"> 每日计划量编辑</a></span></span></strong><asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" CellPadding="5" style="text-align: center" Font-Names="Verdana" Font-Size="12px"
                 ...
                 <Columns >
                    <asp:TemplateField  HeaderText ="菜名">
                        <ItemTemplate  >
                             <asp:Label ID ="cm"    runat="server" Text ='<%#  Eval("cm") %>'/>
                        </ItemTemplate>
                        <EditItemTemplate>  
                            <asp:TextBox ID="T_cmEdit" runat="server" Text='<%# Eval("cm") %>' ></asp:TextBox>  
                        </EditItemTemplate>  
                        <ItemStyle Width="35%"   />
                    </asp:TemplateField>
                    <asp:TemplateField  HeaderText ="单价">
                        <ItemTemplate  >
                             <asp:Label ID ="dj"    runat="server" Text ='<%#  Eval("dj") %>'/>
                        </ItemTemplate>
                        <EditItemTemplate>  
                            <asp:TextBox ID="T_djEdit" runat="server" Text='<%# Eval("dj") %>' Width="100px" ></asp:TextBox>  
                        </EditItemTemplate>  
                        <ItemStyle Width="15%"   />
                    </asp:TemplateField>
            ...
            <table width="800px" class ="T_Sample1"  align="center" style="font-size: 9pt">
                <tr>
                    <td style="width:35%"  align="center"><asp:TextBox  ID ="T_CM" runat="server" Width="200px"   onchange="javascript:cmcz(this.value);" /></td>
                    <td style="width:15%" align="center"><asp:TextBox  ID ="T_DJ" runat="server" Width="100px" onkeyup="clearNoNum(this)"  Text="0.00" /></td>
                    <td style="width:15%" align="center"><asp:TextBox  ID ="T_SL" runat="server" Width="100px" onkeyup="clearNoNum(this)" Text="300"  /></td>
                    <td style="width:15%" align="center"><asp:TextBox  ID ="T_RQ" runat="server" Width="100px" /></td>
                    <td style="width:10%" align="center"><asp:Button ID="B_TJ" runat="server" Text="添 加"  OnClick="B_TJ_Click" OnClientClick=" javaScript:return IntoClick()"/></td>
                </tr>
            </table>
