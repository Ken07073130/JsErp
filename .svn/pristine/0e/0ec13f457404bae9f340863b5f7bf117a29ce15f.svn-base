<%@ Page Language="C#" AutoEventWireup="true" CodeFile="cpkftaEdit.aspx.cs" Inherits="cpkftaEdit" %>
...
function lbtnOK_disbale(){
  if (document.getElementById('<%= ddlXsry.ClientID %>').value.length>0 && document.getElementById('<%= tbXsryhqsj.ClientID %>').value.length<=0 &&document.getElementById('<%= tbLchqzt.ClientID %>').value=="退回业务员处理"){
         var sysDate=new Date(); 
         document.getElementById('<%= tbXsryhqsj.ClientID %>').value=sysDate.getFullYear()+"-"+(sysDate.getMonth()+1)+"-"+sysDate.getDate(); 
         //初始化会签状态
         document.getElementById('<%= tbLchqzt.ClientID %>').value="业务员已修改";
  } 
  if (document.getElementById('<%= ddlXsry.ClientID %>').value.length>0 && document.getElementById('<%= tbXsryhqsj.ClientID %>').value.length<=0 &&document.getElementById('<%= tbLchqzt.ClientID %>').value=="产品开发提案同步"){
         var sysDate=new Date(); 
         document.getElementById('<%= tbXsryhqsj.ClientID %>').value=sysDate.getFullYear()+"-"+(sysDate.getMonth()+1)+"-"+sysDate.getDate(); 
         //初始化会签状态
         document.getElementById('<%= tbLchqzt.ClientID %>').value="销售经理会签中";
         document.getElementById('<%= ddlXsjlhqzt.ClientID %>').value="会签中";
         document.getElementById('<%= ddlSwjlhqzt.ClientID %>').value="-";
         document.getElementById('<%= ddlZghqzt.ClientID %>').value="-";
        ...
        <div style="font-size: 12px;">
            ...
            <table width="760" border="0" cellspacing="0" cellpadding="0" align="center" bgcolor="#F1FEDD"
                ...
                <tr style="font-size: 25px; font-weight: bold;">
                    <td colspan="12" >
                        <label id="lblTitle" runat="server">产品开发提案书</label>
                        ...
                        <a style="color: #000; padding: 6px 3px; display: inline-block; text-decoration: none;
                            background-color: #529F1D; font-size: 12px" href="cpkftaPrint.aspx?djlx=<%=djlx%>&xh=<%=xh%>&lb=EDIT&editType=全部"
                            target="_blank">打印</a>
                    </td>
                </tr>
                <tr>
          ...
          <table width="760"  border="0" cellspacing="0" cellpadding="0" align="center" bgcolor="#F1FEDD"
              ...
              <tr>
                <td style="height: 37px" class="tdNeed">
                        成品状态(＊)&nbsp;</td>
                <td style="height: 37px">&nbsp;<asp:DropDownList ID="ddlCpzt" runat="server" onchange="cpztChange()">
                            <asp:ListItem>请选择</asp:ListItem>
              ...
              <tr>
                <td style="height: 48px">&nbsp;成品尺寸类型</td>
                <td style="height: 48px" colspan="2">&nbsp;&nbsp;<asp:DropDownList ID="ddlCpcclx" runat="server" Width="90%" onchange="cpcclxChange()">
                            <asp:ListItem>请选择</asp:ListItem>
                            <asp:ListItem>初始尺寸</asp:ListItem>
                            <asp:ListItem>循环后尺寸</asp:ListItem>
                        </asp:DropDownList></td>
                <td style="height: 48px" colspan="2">&nbsp; 循环周期<asp:TextBox ID="tbXhzq" runat="server" Width="32px"></asp:TextBox>周</td>
            ...
            <table width="760" border="0" cellspacing="0" cellpadding="0" align="center" bgcolor="#F1FEDD"
                ...
                <tr>
                    <td style="height: 47px" colspan="3" class="tdNeed">
                        样品建议书(＊)</td>
                    <td colspan="2" style="height: 47px">
                ...
                <tr>
                    ...
                    <td style="height: 47px" class="tdNeed">
                        制作情况(＊)</td>
                    <td style="height: 47px" colspan="2">
